...
const error = (msg, status = 400, origin) =>
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (path === "/api/health") {
      return json(
        { status: "ok", tools: Object.keys(TOOL_PROCESSORS).length },
        200,
        origin,
      );
    }
  ...
  try {
    ...
    if (contentType.includes("multipart/form-data")) {
      ...
      if (file.size > maxBytes) {
        return error(
          `File too large. Max ${env.MAX_FILE_MB || 500}MB`,
          413,
          origin,
        );
      }
...
async function callFFmpeg({ env, inputKey, command, outputFileName }) {
  if (!env.FFMPEG_API_URL) {
    throw new Error(
      "FFMPEG_API_URL not configured. See worker.js setup instructions.",
    );
  }
...
async function processVideoConverter({
  env,
  inputKey,
  options,
  tool,
  ...
  const outputExt =
    options.outputFormat || tool.replace("-converter", "") || "mp4";
...
async function processLoopVideo({ env, inputKey, options, fileName }) {
  const loops =
    options.loops === "Infinite (GIF)" ? 0 : parseInt(options.loops) || 2;
...
async function processAddAudioToVideo({ env, inputKey, options, fileName }) {
  ...
  const replace = options.replaceAudio
    ? "-map 0:v -map 1:a"
    : "-filter_complex amix";
...
async function processAddSubtitlesToVideo({
  env,
  inputKey,
  options,
  fileName,
...
async function processAddStickersToVideo({ env, inputKey, fileName }) {
  ...
  return processAddImageToVideo({
    env,
    inputKey,
    options: { position: "Center", opacity: 100, scale: 25 },
    fileName,
  });
}
...
async function processAudioConverter({
  env,
  inputKey,
  options,
  tool,
  ...
  const outputExt =
    options.outputFormat || tool.replace("-converter", "") || "mp3";
...
async function processImageConverter({
  env,
  inputKey,
  options,
  tool,
  ...
  const outputExt =
    options.outputFormat || tool.replace("-converter", "") || "jpg";
  const baseName = fileName.replace(/\.[^.]+$/, "");
  const outName = `${baseName}.${outputExt}`;
  const quality = options.quality
    ? `-q:v ${Math.round((100 - options.quality) / 5 + 1)}`
    : "";
...
async function processAudioToText({ env, inputKey, options }) {
  ...
  formData.append(
    ...
    options.language === "Auto-detect"
      ? ""
      : options.language?.slice(0, 2).toLowerCase() || "",
  );
...
async function processAutoSubtitleGenerator({
  env,
  inputKey,
  options,
  fileName,
}) {
  const transcript = await processAudioToText({
    env,
    inputKey,
    options: { ...options, format: "SRT" },
  });
...
async function processTranslateEnglishVideo({
  env,
  inputKey,
  options,
  fileName,
  ...
  const srtResult = await processAutoSubtitleGenerator({
    env,
    inputKey,
    options,
    fileName,
